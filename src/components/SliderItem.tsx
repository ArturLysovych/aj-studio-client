...
const SliderItem = ({ className, gen_text, gen_bold_text, p_text }: { className: string, gen_text: string, gen_bold_text: string, p_text: string }): JSX.Element => {
    return (
        <div className={`w-fullx h-full ${className} flex flex-col p-[30px] rounded-3xl sm:px-[40px] sm:py-[44px] lg:px-[73px]`}>
            <div className="h-full w-full flex justify-between items-center">
                <div className="h-full w-[50%] flex flex-col items-start justify-start sm:justify-center gap-[10px]">
                    <h2 className='text-[16px] sm:text-[32px] lg:text-[52px]'>{ gen_text } <span className='font-bold'>{ gen_bold_text }</span></h2>
                    <p className='text-[12px] sm:text-[16px] lg:text-[20px]'>{ p_text }</p>
                    <button className={`bg-black px-[20px] py-[8px] sm:text-[16px] lg:text-[20px] text-white text-[14px] lg:rounded-[30px] rounded-3xl shadow-black shadow-2x`}>Discover</button>
                </div>
